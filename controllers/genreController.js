...
exports.genre_delete_post = function(req, res) {
    async.parallel({
        ...
        genres_books: function(callback) {
            Book.find({ 'genre': req.body.authorid })
                .exec(callback);
        }
    }, function(err, results) {
        if (err) { return next(err); }
        ...
        if (results.genres_books.length > 0) {
            res.render('genres_delete', { title: 'Delete Genre', genre: results.genre, genre_books: results.genres_books });
            return;
        }
        else {
